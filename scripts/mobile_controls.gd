...
func _on_next_day_pressed() -> void:
	if farm_manager == null:
		_on_feedback("Sistem kebun belum siap.")
		return
	var result: Dictionary = farm_manager.next_day()
...
func _on_tool_changed(tool: String) -> void:
	...
	hint_label.text = "Lembah Sari 0.0.2  •  %s dipilih  •  A = gunakan" % labels.get(tool, tool)
...
func _refresh_status() -> void:
	if farm_manager != null and farm_manager.has_method("get_status_text"):
		status_label.text = farm_manager.get_status_text()
	else:
		status_label.text = "Hari 1  •  Cabai 0"
