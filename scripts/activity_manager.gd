...
func _use_irrigation() -> Dictionary:
	...
	if watered_count <= 0:
		return {"ok": true, "message": "Pintu irigasi dibuka, tapi belum ada tanaman yang perlu disiram."}

	last_irrigation_day = current_day
	return {"ok": true, "message": "Irigasi dibuka. %d tanaman tersiram sekaligus." % watered_count}
...
func _fish() -> Dictionary:
	...
	return {"ok": true, "message": "Dapat %s! Ikan masuk ke tas." % fish_name}
